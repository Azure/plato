...
class SimpleAdder(Env):
    ...
    def bonsai_reset(self, *, seed=None, options=None):
        obs_dict, info = self.reset(seed=seed, options=options)
        return obs_dict
    
    def bonsai_step(self, action):
        action["addend"] = np.array(action["addend"])
        obs_dict, reward, terminated, truncated, info = self.step(action)
